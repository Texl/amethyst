...
//------------------------------------------------------------------------------
Color
Scene::GetPixelColor(int const screenWidth, 
...
//    return mSceneObjects[index]->mMaterial.mColor;
//    return Color(0.5f * normal.x + 0.5f, 0.5f * normal.y + 0.5f, 0.5f * normal.z + 0.5f);
...
//------------------------------------------------------------------------------
bool
Scene::_FindShadowIntersection(Ray const & ray, float const tLimit)
{
    float tIntersection;

    for (unsigned int i = 0; i < mSceneObjects.size(); ++i)
    {
        if (mSceneObjects[i]->RayCast(ray, tIntersection) && tIntersection <= tLimit)
        {
            return true;
        }
    }
...
//------------------------------------------------------------------------------
bool
Scene::_FindNearestIntersection(unsigned int & index, 
                                Ray const & ray, 
                                Vec3f & point, 
                                Vec3f & normal)
{
    ...
    for (unsigned int i = 0; i < mSceneObjects.size(); ++i)
    {
        if (mSceneObjects[i]->RayCast(ray, t, tMinPoint, tMinNormal))
        {
            if (t < tMin)
            {
                tMin = t;
...
//------------------------------------------------------------------------------
Color
Scene::_GetLocalIllumination(unsigned int index, 
                             Ray const & ray, 
                             Vec3f const & point, 
                             Vec3f const & normal, 
                             float setKs)
{
    ...
    Color const Kd = mSceneObjects[index]->mMaterial.mColor;
    float const Ks = setKs > 0 ? setKs : mSceneObjects[index]->mMaterial.mSpecularCoefficient;
    float const ns = mSceneObjects[index]->mMaterial.mSpecularExponent;
    ...
    for (unsigned int i = 0; i < mLights.size(); i++)
    {
        ...
        Vec3f const lightVector = (mLights[i]->p - point);
        Vec3f const lightDirection = lightVector.normalized();
        float const lightDistance = lightVector.length();
        ...
        switch(AppSettings::mShadowMode)
        {
            case AppSettings::kShadowMode_Hard:
                ...
                if (_FindShadowIntersection(Ray(point + normal * (float)EPSILON, lightDirection), lightDistance))
                {
                    continue;
                }
                break;
        ...
        if (AppSettings::mUseSpecular)
        {
            ...
            Vec3f const reflectionVector = (normal * 2.0f * dotNL - lightDirection).normalized();
            Vec3f const viewVector = (ray.getDirection() * -1).normalized();

            float const dotRV = math<float>::max(0.0f, reflectionVector.dot(viewVector));
